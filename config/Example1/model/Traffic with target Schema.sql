...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.ST_Street', 'U') IS NULL
CREATE TABLE [target].[ST_Street] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.ST_NAM_Street_Name', 'U') IS NULL
CREATE TABLE [target].[ST_NAM_Street_Name] (
    ...
    constraint fkST_NAM_Street_Name foreign key (
        ST_NAM_ST_ID
    ) references [target].[ST_Street](ST_ID),
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.IS_Intersection', 'U') IS NULL
CREATE TABLE [target].[IS_Intersection] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.IS_COL_Intersection_CollisionCount', 'U') IS NULL
CREATE TABLE [target].[IS_COL_Intersection_CollisionCount] (
    ...
    constraint fkIS_COL_Intersection_CollisionCount foreign key (
        IS_COL_IS_ID
    ) references [target].[IS_Intersection](IS_ID),
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.IS_INJ_Intersection_InjuredCount', 'U') IS NULL
CREATE TABLE [target].[IS_INJ_Intersection_InjuredCount] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.IS_KIL_Intersection_KilledCount', 'U') IS NULL
CREATE TABLE [target].[IS_KIL_Intersection_KilledCount] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.IS_VEH_Intersection_VehicleCount', 'U') IS NULL
CREATE TABLE [target].[IS_VEH_Intersection_VehicleCount] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.ST_intersecting_IS_of_ST_crossing', 'U') IS NULL
CREATE TABLE [target].[ST_intersecting_IS_of_ST_crossing] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rfIS_COL_Intersection_CollisionCount', 'FN') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rfIS_COL_Intersection_CollisionCount] (
        @id int,
        @value int,
        @changed date
    )
        ...
        CASE WHEN EXISTS (
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_COL_Intersection_CollisionCount] pre
                    WHERE
                        pre.IS_COL_IS_ID = @id
                    AND
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_COL_Intersection_CollisionCount] fol
                    WHERE
                        fol.IS_COL_IS_ID = @id
                    AND
    ...
    END
    ');
    ALTER TABLE [target].[IS_COL_Intersection_CollisionCount]
    ADD CONSTRAINT [rcIS_COL_Intersection_CollisionCount] CHECK (
        [target].[rfIS_COL_Intersection_CollisionCount] (
            IS_COL_IS_ID,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rfIS_INJ_Intersection_InjuredCount', 'FN') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rfIS_INJ_Intersection_InjuredCount] (
        @id int,
        @value int,
        @changed date
    )
        ...
        CASE WHEN EXISTS (
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_INJ_Intersection_InjuredCount] pre
                    WHERE
                        pre.IS_INJ_IS_ID = @id
                    AND
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_INJ_Intersection_InjuredCount] fol
                    WHERE
                        fol.IS_INJ_IS_ID = @id
                    AND
    ...
    END
    ');
    ALTER TABLE [target].[IS_INJ_Intersection_InjuredCount]
    ADD CONSTRAINT [rcIS_INJ_Intersection_InjuredCount] CHECK (
        [target].[rfIS_INJ_Intersection_InjuredCount] (
            IS_INJ_IS_ID,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rfIS_KIL_Intersection_KilledCount', 'FN') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rfIS_KIL_Intersection_KilledCount] (
        @id int,
        @value int,
        @changed date
    )
        ...
        CASE WHEN EXISTS (
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_KIL_Intersection_KilledCount] pre
                    WHERE
                        pre.IS_KIL_IS_ID = @id
                    AND
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_KIL_Intersection_KilledCount] fol
                    WHERE
                        fol.IS_KIL_IS_ID = @id
                    AND
    ...
    END
    ');
    ALTER TABLE [target].[IS_KIL_Intersection_KilledCount]
    ADD CONSTRAINT [rcIS_KIL_Intersection_KilledCount] CHECK (
        [target].[rfIS_KIL_Intersection_KilledCount] (
            IS_KIL_IS_ID,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rfIS_VEH_Intersection_VehicleCount', 'FN') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rfIS_VEH_Intersection_VehicleCount] (
        @id int,
        @value smallint,
        @changed date
    )
        ...
        CASE WHEN EXISTS (
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_VEH_Intersection_VehicleCount] pre
                    WHERE
                        pre.IS_VEH_IS_ID = @id
                    AND
            ...
            WHERE
                @value = (
                    ...
                    FROM
                        [target].[IS_VEH_Intersection_VehicleCount] fol
                    WHERE
                        fol.IS_VEH_IS_ID = @id
                    AND
    ...
    END
    ');
    ALTER TABLE [target].[IS_VEH_Intersection_VehicleCount]
    ADD CONSTRAINT [rcIS_VEH_Intersection_VehicleCount] CHECK (
        [target].[rfIS_VEH_Intersection_VehicleCount] (
            IS_VEH_IS_ID,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.kST_Street', 'P') IS NULL
BEGIN
    EXEC('
    CREATE PROCEDURE [target].[kST_Street] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.kIS_Intersection', 'P') IS NULL
BEGIN
    EXEC('
    CREATE PROCEDURE [target].[kIS_Intersection] (
        @requestedNumberOfIdentities bigint,
        @metadata int
    ) AS
    BEGIN
        ...
        BEGIN
            ...
            INSERT INTO [target].[IS_Intersection] (
                Metadata_IS
            )
            OUTPUT
                inserted.IS_ID
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rIS_COL_Intersection_CollisionCount','IF') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rIS_COL_Intersection_CollisionCount] (
    ...
    SELECT
        ...
        IS_COL_ChangedAt
    FROM
        [target].[IS_COL_Intersection_CollisionCount]
    WHERE
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rIS_INJ_Intersection_InjuredCount','IF') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rIS_INJ_Intersection_InjuredCount] (
        @changingTimepoint date
    )
    RETURNS TABLE WITH SCHEMABINDING AS RETURN
    SELECT
        ...
        IS_INJ_ChangedAt
    FROM
        [target].[IS_INJ_Intersection_InjuredCount]
    WHERE
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rIS_KIL_Intersection_KilledCount','IF') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rIS_KIL_Intersection_KilledCount] (
        @changingTimepoint date
    )
    RETURNS TABLE WITH SCHEMABINDING AS RETURN
    SELECT
        ...
        IS_KIL_ChangedAt
    FROM
        [target].[IS_KIL_Intersection_KilledCount]
    WHERE
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.rIS_VEH_Intersection_VehicleCount','IF') IS NULL
BEGIN
    EXEC('
    CREATE FUNCTION [target].[rIS_VEH_Intersection_VehicleCount] (
        @changingTimepoint date
    )
    RETURNS TABLE WITH SCHEMABINDING AS RETURN
    SELECT
        ...
        IS_VEH_ChangedAt
    FROM
        [target].[IS_VEH_Intersection_VehicleCount]
    WHERE
...
--
-- Drop perspectives --------------------------------------------------------------------------------------------------
IF Object_ID('target.dST_Street', 'IF') IS NOT NULL
DROP FUNCTION [target].[dST_Street];
IF Object_ID('target.nST_Street', 'V') IS NOT NULL
DROP VIEW [target].[nST_Street];
IF Object_ID('target.pST_Street', 'IF') IS NOT NULL
DROP FUNCTION [target].[pST_Street];
IF Object_ID('target.lST_Street', 'V') IS NOT NULL
DROP VIEW [target].[lST_Street];
GO
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lST_Street viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE VIEW [target].[lST_Street] WITH SCHEMABINDING AS
SELECT
    [ST].ST_ID,
    [ST].Metadata_ST,
    [NAM].ST_NAM_ST_ID,
...
-----------------------------------------------------------------------------------------------------------------------
CREATE FUNCTION [target].[pST_Street] (
...
-----------------------------------------------------------------------------------------------------------------------
CREATE VIEW [target].[nST_Street]
AS
SELECT
    *
FROM
    [target].[pST_Street](sysdatetime());
GO
-- Drop perspectives --------------------------------------------------------------------------------------------------
IF Object_ID('target.dIS_Intersection', 'IF') IS NOT NULL
DROP FUNCTION [target].[dIS_Intersection];
IF Object_ID('target.nIS_Intersection', 'V') IS NOT NULL
DROP VIEW [target].[nIS_Intersection];
IF Object_ID('target.pIS_Intersection', 'IF') IS NOT NULL
DROP FUNCTION [target].[pIS_Intersection];
IF Object_ID('target.lIS_Intersection', 'V') IS NOT NULL
DROP VIEW [target].[lIS_Intersection];
GO
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lIS_Intersection viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE VIEW [target].[lIS_Intersection] WITH SCHEMABINDING AS
SELECT
    [IS].IS_ID,
    [IS].Metadata_IS,
    [COL].IS_COL_IS_ID,
...
FROM
    [target].[IS_Intersection] [IS]
LEFT JOIN
    [target].[IS_COL_Intersection_CollisionCount] [COL]
ON
...
AND
    [COL].IS_COL_ChangedAt = (
        ...
        FROM
            [target].[IS_COL_Intersection_CollisionCount] sub
        WHERE
            sub.IS_COL_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[IS_INJ_Intersection_InjuredCount] [INJ]
ON
...
AND
    [INJ].IS_INJ_ChangedAt = (
        ...
        FROM
            [target].[IS_INJ_Intersection_InjuredCount] sub
        WHERE
            sub.IS_INJ_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[IS_KIL_Intersection_KilledCount] [KIL]
ON
...
AND
    [KIL].IS_KIL_ChangedAt = (
        ...
        FROM
            [target].[IS_KIL_Intersection_KilledCount] sub
        WHERE
            sub.IS_KIL_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[IS_VEH_Intersection_VehicleCount] [VEH]
ON
...
AND
    [VEH].IS_VEH_ChangedAt = (
        ...
        FROM
            [target].[IS_VEH_Intersection_VehicleCount] sub
        WHERE
...
-----------------------------------------------------------------------------------------------------------------------
CREATE FUNCTION [target].[pIS_Intersection] (
    @changingTimepoint datetime2(7)
)
RETURNS TABLE WITH SCHEMABINDING AS RETURN
SELECT
...
FROM
    [target].[IS_Intersection] [IS]
LEFT JOIN
    [target].[rIS_COL_Intersection_CollisionCount](@changingTimepoint) [COL]
ON
    [COL].IS_COL_IS_ID = [IS].IS_ID
AND
    [COL].IS_COL_ChangedAt = (
        SELECT
            max(sub.IS_COL_ChangedAt)
        FROM
            [target].[rIS_COL_Intersection_CollisionCount](@changingTimepoint) sub
        WHERE
            sub.IS_COL_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[rIS_INJ_Intersection_InjuredCount](@changingTimepoint) [INJ]
ON
    [INJ].IS_INJ_IS_ID = [IS].IS_ID
AND
    [INJ].IS_INJ_ChangedAt = (
        SELECT
            max(sub.IS_INJ_ChangedAt)
        FROM
            [target].[rIS_INJ_Intersection_InjuredCount](@changingTimepoint) sub
        WHERE
            sub.IS_INJ_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[rIS_KIL_Intersection_KilledCount](@changingTimepoint) [KIL]
ON
    [KIL].IS_KIL_IS_ID = [IS].IS_ID
AND
    [KIL].IS_KIL_ChangedAt = (
        SELECT
            max(sub.IS_KIL_ChangedAt)
        FROM
            [target].[rIS_KIL_Intersection_KilledCount](@changingTimepoint) sub
        WHERE
            sub.IS_KIL_IS_ID = [IS].IS_ID
   )
LEFT JOIN
    [target].[rIS_VEH_Intersection_VehicleCount](@changingTimepoint) [VEH]
ON
    [VEH].IS_VEH_IS_ID = [IS].IS_ID
AND
    [VEH].IS_VEH_ChangedAt = (
        SELECT
            max(sub.IS_VEH_ChangedAt)
        FROM
            [target].[rIS_VEH_Intersection_VehicleCount](@changingTimepoint) sub
        WHERE
            sub.IS_VEH_IS_ID = [IS].IS_ID
   );
GO
...
CROSS APPLY
    [target].[pIS_Intersection](timepoints.inspectedTimepoint) [pIS]
WHERE
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_ST_NAM_Street_Name', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_ST_NAM_Street_Name];
GO
CREATE TRIGGER [target].[it_ST_NAM_Street_Name] ON [target].[ST_NAM_Street_Name]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        INSERT INTO [target].[ST_NAM_Street_Name] (
            ST_NAM_ST_ID,
            Metadata_ST_NAM,
            ST_NAM_Street_Name
        )
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_IS_COL_Intersection_CollisionCount', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_IS_COL_Intersection_CollisionCount];
GO
CREATE TRIGGER [target].[it_IS_COL_Intersection_CollisionCount] ON [target].[IS_COL_Intersection_CollisionCount]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        SET
            v.IS_COL_StatementType =
                CASE
                    ...
                    THEN 'D' -- duplicate
                    WHEN [target].[rfIS_COL_Intersection_CollisionCount](
                        v.IS_COL_IS_ID,
        ...
        INSERT INTO [target].[IS_COL_Intersection_CollisionCount] (
            IS_COL_IS_ID,
            Metadata_IS_COL,
            IS_COL_ChangedAt,
            IS_COL_Intersection_CollisionCount
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_IS_INJ_Intersection_InjuredCount', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_IS_INJ_Intersection_InjuredCount];
GO
CREATE TRIGGER [target].[it_IS_INJ_Intersection_InjuredCount] ON [target].[IS_INJ_Intersection_InjuredCount]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        SET
            v.IS_INJ_StatementType =
                CASE
                    ...
                    THEN 'D' -- duplicate
                    WHEN [target].[rfIS_INJ_Intersection_InjuredCount](
                        v.IS_INJ_IS_ID,
        ...
        INSERT INTO [target].[IS_INJ_Intersection_InjuredCount] (
            IS_INJ_IS_ID,
            Metadata_IS_INJ,
            IS_INJ_ChangedAt,
            IS_INJ_Intersection_InjuredCount
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_IS_KIL_Intersection_KilledCount', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_IS_KIL_Intersection_KilledCount];
GO
CREATE TRIGGER [target].[it_IS_KIL_Intersection_KilledCount] ON [target].[IS_KIL_Intersection_KilledCount]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        SET
            v.IS_KIL_StatementType =
                CASE
                    ...
                    THEN 'D' -- duplicate
                    WHEN [target].[rfIS_KIL_Intersection_KilledCount](
                        v.IS_KIL_IS_ID,
        ...
        INSERT INTO [target].[IS_KIL_Intersection_KilledCount] (
            IS_KIL_IS_ID,
            Metadata_IS_KIL,
            IS_KIL_ChangedAt,
            IS_KIL_Intersection_KilledCount
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_IS_VEH_Intersection_VehicleCount', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_IS_VEH_Intersection_VehicleCount];
GO
CREATE TRIGGER [target].[it_IS_VEH_Intersection_VehicleCount] ON [target].[IS_VEH_Intersection_VehicleCount]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        SET
            v.IS_VEH_StatementType =
                CASE
                    ...
                    THEN 'D' -- duplicate
                    WHEN [target].[rfIS_VEH_Intersection_VehicleCount](
                        v.IS_VEH_IS_ID,
        ...
        INSERT INTO [target].[IS_VEH_Intersection_VehicleCount] (
            IS_VEH_IS_ID,
            Metadata_IS_VEH,
            IS_VEH_ChangedAt,
            IS_VEH_Intersection_VehicleCount
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[it_lST_Street] ON [target].[lST_Street]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[ut_lST_Street] ON [target].[lST_Street]
INSTEAD OF UPDATE
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[dt_lST_Street] ON [target].[lST_Street]
INSTEAD OF DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[it_lIS_Intersection] ON [target].[lIS_Intersection]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[ut_lIS_Intersection] ON [target].[lIS_Intersection]
INSTEAD OF UPDATE
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[dt_lIS_Intersection] ON [target].[lIS_Intersection]
INSTEAD OF DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
--
-- Drop perspectives --------------------------------------------------------------------------------------------------
IF Object_ID('target.dST_intersecting_IS_of_ST_crossing', 'IF') IS NOT NULL
DROP FUNCTION [target].[dST_intersecting_IS_of_ST_crossing];
IF Object_ID('target.nST_intersecting_IS_of_ST_crossing', 'V') IS NOT NULL
DROP VIEW [target].[nST_intersecting_IS_of_ST_crossing];
IF Object_ID('target.pST_intersecting_IS_of_ST_crossing', 'IF') IS NOT NULL
DROP FUNCTION [target].[pST_intersecting_IS_of_ST_crossing];
IF Object_ID('target.lST_intersecting_IS_of_ST_crossing', 'V') IS NOT NULL
DROP VIEW [target].[lST_intersecting_IS_of_ST_crossing];
GO
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lST_intersecting_IS_of_ST_crossing viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE VIEW [target].[lST_intersecting_IS_of_ST_crossing] WITH SCHEMABINDING AS
SELECT
...
-----------------------------------------------------------------------------------------------------------------------
CREATE VIEW [target].[nST_intersecting_IS_of_ST_crossing]
AS
SELECT
    *
FROM
    [target].[pST_intersecting_IS_of_ST_crossing](sysdatetime());
GO
-- TIE TRIGGERS -------------------------------------------------------------------------------------------------------
--
-- The following triggers on the latest view make it behave like a table.
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target.it_ST_intersecting_IS_of_ST_crossing', 'TR') IS NOT NULL
DROP TRIGGER [target].[it_ST_intersecting_IS_of_ST_crossing];
GO
CREATE TRIGGER [target].[it_ST_intersecting_IS_of_ST_crossing] ON [target].[ST_intersecting_IS_of_ST_crossing]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INSERT INTO [target].[ST_intersecting_IS_of_ST_crossing] (
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[it_lST_intersecting_IS_of_ST_crossing] ON [target].[lST_intersecting_IS_of_ST_crossing]
INSTEAD OF INSERT
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[ut_lST_intersecting_IS_of_ST_crossing] ON [target].[lST_intersecting_IS_of_ST_crossing]
INSTEAD OF UPDATE
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
CREATE TRIGGER [target].[dt_lST_intersecting_IS_of_ST_crossing] ON [target].[lST_intersecting_IS_of_ST_crossing]
INSTEAD OF DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Schema', 'U') IS NULL
   CREATE TABLE [target].[_Schema] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Schema_Expanded', 'V') IS NOT NULL
DROP VIEW [target].[_Schema_Expanded]
GO
CREATE VIEW [target].[_Schema_Expanded]
AS
SELECT
	[version],
	[activation],
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Anchor', 'V') IS NOT NULL
DROP VIEW [target].[_Anchor]
GO
CREATE VIEW [target].[_Anchor]
AS
SELECT
   S.version,
   S.activation,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Knot', 'V') IS NOT NULL
DROP VIEW [target].[_Knot]
GO
CREATE VIEW [target].[_Knot]
AS
SELECT
   S.version,
   S.activation,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Attribute', 'V') IS NOT NULL
DROP VIEW [target].[_Attribute]
GO
CREATE VIEW [target].[_Attribute]
AS
SELECT
   S.version,
   S.activation,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Tie', 'V') IS NOT NULL
DROP VIEW [target].[_Tie]
GO
CREATE VIEW [target].[_Tie]
AS
SELECT
   S.version,
   S.activation,
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._Evolution', 'IF') IS NOT NULL
DROP FUNCTION [target].[_Evolution];
GO
CREATE FUNCTION [target].[_Evolution] (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._GenerateDropScript', 'P') IS NOT NULL
DROP PROCEDURE [target].[_GenerateDropScript];
GO
CREATE PROCEDURE [target]._GenerateDropScript (
...
-----------------------------------------------------------------------------------------------------------------------
IF Object_ID('target._GenerateCopyScript', 'P') IS NOT NULL
DROP PROCEDURE [target].[_GenerateCopyScript];
GO
CREATE PROCEDURE [target]._GenerateCopyScript (
	@source varchar(123),
	@target varchar(123)
)
as
